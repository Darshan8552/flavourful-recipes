import { getUserFavoriteRecipes } from "@/actions/favorite-recipes-action";
import RecipeCard from "@/components/recipes/recipes-card";
import Pagination from "@/components/recipes/pagination";
import { Heart, Search } from "lucide-react";
import React from "react";

interface FavoriteRecipesContentProps {
  userId: string;
  currentPage: number;
}

const FavoriteRecipesContent: React.FC<FavoriteRecipesContentProps> = async ({
  userId,
  currentPage,
}) => {
  const recipesData = await getUserFavoriteRecipes(userId, currentPage, 12);
    ...
    return (
      <div className="flex flex-col items-center justify-center w-full h-64 text-center">
        <div className="bg-gray-200 dark:bg-zinc-800 rounded-full p-6 mb-4">
          <Heart className="w-12 h-12 text-gray-600 dark:text-zinc-400" />
        </div>
        ...
        <p className="text-gray-600 dark:text-zinc-400 mb-4 max-w-md">
          Start exploring our recipes and mark your favorites by clicking the
          heart icon on recipes you love.
        </p>
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-zinc-400">
          <Search className="w-4 h-4" />
          <span>Discover recipes to add to your favorites</span>
        </div>
      </div>
    );
  }
        ...
        <div>
          <p className="text-gray-600 dark:text-zinc-400 text-sm">
            {recipesData.totalCount > 0
              ? `Showing ${recipesData.recipes.length} of ${recipesData.totalCount} favorite recipes`
